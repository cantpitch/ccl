...
/*  Apple/ARM64 ABI register conventions:
    - x0-x7 foreign (C) function parameters and results (the rest on stack)
    - x8 indirect result allocated by caller for returning structs
    - x9-x15 corruptible registers
    - x16-x17 intra-procedure call scratch registers
    - x18 platform register (PR)
...
/* macos uses x18 (Platform Register) https://stackoverflow.com/questions/71152539/consequence-of-violating-macoss-arm64-calling-convention 
   This register may be corrupted between task switches */
define(`save2',`x18')
define(`save3',`x19')
define(`save4',`x20')
define(`save5',`x21')
