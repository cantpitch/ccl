...
#define NATIVE_FLOAT_STATE_FLAVOR ARM_VFP_STATE
...
int is_conditional_trap(opcode);

#define kNameBufLen 256
#define TRAP_LOOKUP_TRIES 5   /* # instrs to scan before trap instr */

void callback_for_trap (LispObj, ExceptionInformation *, pc, natural, natural, natural);

natural register_codevector_contains_pc (natural, pc);

void callback_to_lisp (LispObj, ExceptionInformation *, natural, natural, natural, natural, natural);

OSStatus handle_trap(ExceptionInformation *, opcode, pc, siginfo_t *);

unsigned scan_for_instr( unsigned, unsigned, pc );
